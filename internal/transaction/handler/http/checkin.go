...
func (h *checkInHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		log.Printf("[CheckIn HTTP][checkInHandler] Failed to parse transaction ID. ID: %s. Err: %s\n", vars["id"], err.Error())
...
func (h *checkInHandler) handleUpdateCheckInStatus(w http.ResponseWriter, r *http.Request, id int64) {
	...
	defer func() {
		if err != nil {
			log.Printf("[CheckIn HTTP][handleUpdateCheckInStatus] Failed to update check in status. transactionID: %v. Source: %s, Err: %s\n", id, source, err.Error())
	...
	go func() {
		...
		res, err := h.transaction.UpdateCheckInStatus(ctx, id, ticketNumber)
		if err != nil {
